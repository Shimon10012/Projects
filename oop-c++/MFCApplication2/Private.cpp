#include "pch.h"
#include "Private.h"
IMPLEMENT_SERIAL(Private, CObject, 1)

Private::Private(int fuel_amount, int care_month, int type_of_vehicle, int price, int speed, int capacity, CString name)
{
...
int Private::print(CListCtrl& vehicle_view,int nItem)
{
	{int nItem;
		char arr[1000];
		...
		vehicle_view.SetItemText(nItem, 1, L"private");
...
void Private::set_price(int price)
{
	this->price = price;

}

void Private::set_max_speed(int maxspeed)
{
	this->max_speed = maxspeed;
}

void Private::set_max_people(int maxpeople)
{
	this->max_capacity = maxpeople;
}

void Private::Serialize(CArchive& arr)
{
